...
export const scrapeWebsite = async (req, res, next) => {
  ...
  try {
    ...
    const facebook = socialMediaLinks.find((link) =>
      link.includes("facebook.com")
    );
    const twitter = socialMediaLinks.find((link) =>
      link.includes("twitter.com")
    );
    const linkedIn = socialMediaLinks.find((link) =>
    ...
    const faviconUrl = await page.evaluate(() => {
      ...
      const faviconElement = Array.from(linkElements).find((el) =>
    ...
    const contactInfo = await page.evaluate(() => {
      ...
      const phonePattern =
        /(?:\+?\d{1,3}[ -]?)?(?:\(?\d{2,4}\)?[ -]?)?\d{9,10}/g; // Updated regex for 9-10 digit phone numbers
      const emailPattern = /([\w.-]+@(?=[a-z\d][^.]*\.)[a-z\d.-]*[^.])/g; // Improved regex for email extraction
      const phones = bodyText.match(phonePattern) || [];
      const emails = bodyText.match(emailPattern) || [];
      return {
        phone: phones.length > 0 ? phones[0] : null, // Get first phone number
        email: emails.length > 0 ? emails[0] : null, // Get first email
      };
    });
    ...
    const addresses = await page.evaluate(() => {
      ...
      const addressPattern =
        /(\d+\s)?([A-Za-z]+\.?\s){1,5}(Ave|Avenue|St|Street|Rd|Road|Blvd|Boulevard|Ln|Lane|Dr|Drive|Ct|Court|Pl|Place|Way|Pkwy|Parkway|Cres|Crescent|Sq|Square|Circle|Cir|Terr|Terrace|Pz|Plaza|Ste|Suite|Bldg|Building|Floor|Fl|Unit|Apt|Apartment|Rm|Room)\s?(\d{1,5})?((\n)|(\,\s)|(\,\n)|(\s\w{2,3}\s\d{5})|(\n\d{5}))/gim;
...
export const deleteScrapes = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to delete scrapes", error: error.message });
  }
};
